<template id="sportsbook-table">
  <b-table v-if="tableData" class="px-0 mx-0" ref="table" id="sports-book-table"
    v-bind:class="{'table-not-stacked': !isStacked}" small noCollapse primary-key="id" :no-border-collapse="true"
    ...
    <template v-slot:cell(compaign_name)="data">
      <a class="compaign_name_seo" :name="data.value +' | Casinos'" :href="getByName(data).compaign_url"><img
          :src="'./src/libs/img/'+getByName(data).compaign_img" :alt="data.value" /></a>
    </template>
